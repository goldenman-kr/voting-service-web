  ...
  return (
    <div className="grid gap-6">
      <PageHeader
        eyebrow="선거인명부 관리"
        title="독립 선거인명부"
        description="투표 생성과 분리해 선거인명부를 만들고, 사용 여부에 따라 수정 가능 상태를 관리합니다."
        actions={
            ...
            <tbody className="divide-y divide-slate-200">
              {registries.map((registry) => (
                <tr key={registry.id}>
                  ...
                  <td className="px-4 py-4">{registry.used ? "사용됨" : "미사용"}</td>
                  <td className="px-4 py-4">
                    {registry.editable ? (
                      <span className="rounded-md bg-emerald-50 px-2 py-1 text-xs font-semibold text-emerald-800">수정 가능</span>
                    ) : (
                      <span className="rounded-md bg-slate-100 px-2 py-1 text-xs font-semibold text-slate-700">사용됨 · 수정 불가</span>
                    )}
                  </td>
